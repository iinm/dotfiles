...
agent-sandbox --dockerfile Dockerfile.minimum --idle-timeout 0 bash -c 'echo $(id -u):$(id -g)' | grep -qE "$(id -u):$(id -g)"


echo "case: working directory is mounted and readable"
# when/then:
agent-sandbox --dockerfile Dockerfile.minimum --idle-timeout 0 cat Dockerfile.minimum | grep -qE "FROM debian"


echo "case: working directory owner is sandbox user"
# when/then:
agent-sandbox --dockerfile Dockerfile.minimum --idle-timeout 0 ls -ld . | grep -qE sandbox


echo "case: working directory is read-only by default"
# when:
out=$(agent-sandbox --dockerfile Dockerfile.minimum --idle-timeout 0 touch test 2>&1) || status=$?
# then:
test "$status" -ne 0
...
# when/then:
agent-sandbox --allow-write --dockerfile Dockerfile.minimum --idle-timeout 0 touch test && test -e test
rm -f test
...
nc_pid=$!
# when:
out=$(agent-sandbox --dockerfile Dockerfile.minimum --idle-timeout 0 busybox nc -w 2 host.docker.internal < /dev/null 8000 2>&1) || status=$?
# then:
test "$status" -ne 0
...
nc_pid=$!
# when:
out=$(agent-sandbox --dockerfile Dockerfile.minimum --idle-timeout 0 --allow-net host.docker.internal busybox nc -w 2 host.docker.internal 8000 < /dev/null 2>&1) || status=$?
# then:
...
nc_pid=$!
# when:
out=$(agent-sandbox --dockerfile Dockerfile.minimum --idle-timeout 0 --allow-net host.docker.internal:8000 busybox nc -w 2 host.docker.internal 8000 < /dev/null 2>&1)
# cleanup:
if lsof -i:8000 | grep -q "$nc_pid"; then
  kill "$nc_pid"
fi
...
nc_pid=$!
# when:
out=$(agent-sandbox --dockerfile Dockerfile.minimum --idle-timeout 0 --allow-net 0.0.0.0/0 busybox nc -w 2 8.8.8.8 443 < /dev/null 2>&1)
# cleanup:
if lsof -i:8000 | grep -q "$nc_pid"; then
  kill "$nc_pid"
fi
