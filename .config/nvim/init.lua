...
local setup_keymap = function()
  ...
  vim.keymap.set('n', '<leader>O', ':<C-u>OldfilesGlobal<CR>')
  ...
  vim.keymap.set({ 'n', 'v' }, '<leader>C', ':CopyContextAbs<CR>')
...
local setup_commands = function()
  ...
  vim.api.nvim_create_user_command('Doc', function(opts)
    ...
    if filetype == 'terraform' then
      vim.ui.open('https://registry.terraform.io/search/providers?q=' .. cursor_word)
    else
      vim.ui.open('https://www.google.com/search?q=' .. cursor_word)
    end
  end, { range = true })
...
local setup_minuet = function()
  ...
  local aws_creds = use_bedrock and local_secrets.minuet_aws_profile
      and aws.use_aws_credential(local_secrets.minuet_aws_profile)
  ...
  local base_provider_options = {
    claude = use_bedrock and {
      end_point = local_secrets.minuet_claude_bedrock_endpoint
          or 'https://bedrock-runtime.ap-northeast-1.amazonaws.com/model/global.anthropic.claude-haiku-4-5-20251001-v1:0/invoke',
      optional = {
